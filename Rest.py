...
title_lbl=Label(win,text='Restaurant Billing System',font=('',50,'bold','underline'),bg='skyblue')
...
def editprice_screen():
    ...
    con=connect('resturent.db')
    cur=con.cursor()
    cur.execute("select item_name,item_price from items")
    rows=cur.fetchall()
    items=[]
    for row in rows:
        items.append(row[0])
    ...
    def select_item(event):
        ...
        con=connect('resturent.db')
        cur=con.cursor()
        cur.execute("select item_price from items where item_name=?",(item,))
        row=cur.fetchone()
        iprice=row[0]
        pass_entry.delete(0,END)
    ...
    def updatePrice_db():
        ...
        con=connect("resturent.db")
        cur=con.cursor()
        cur.execute("update items set item_price=? where item_name=?",(price,item))
        con.commit()
        con.close()
...
def billing_screen():
    ...
    items_frm.place(relx=.35,rely=.15,width=800,height=400)
    ...
    item_label.place(relx=.05,rely=.25)
    item_entry.place(relx=.1,rely=.25)
    
    qty_label.place(relx=.05,rely=.35)
    qty_entry.place(relx=.1,rely=.35)

    add_btn.place(relx=.05,rely=.45)
    delete_btn.place(relx=.15,rely=.45)
    ...
    def final_bill():
        ...
        Label(items_frm,text=f"Total Bill:Rs. {amt}",bg='white',fg='black',font=('',14,'bold')).place(relx=.45,rely=.8)
        Label(items_frm,text=f"Total Bill with 5% GST: Rs.{final_amt}",bg='white',fg='black',font=('',14,'bold')).place(relx=.45,rely=.9)
